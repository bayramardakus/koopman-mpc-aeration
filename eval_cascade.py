...
5-tank cascade (koopman_mpc_cascade.py):
...
Usage:  python eval_cascade.py variance   |   python eval_cascade.py mismatch
"""
import sys, json, numpy as np
import plant_model_cascade as PM
import koopman_mpc_cascade as K
...
def variance(seeds=(1,2,3,4,5)):
    ...
    for sd in seeds:
        ...
        mpc = K.KoopMPC(km, w_N=K.W_N_OPERATING)
    ...
    json.dump(stats, open('results_cascade_variance.json','w'), indent=2)
    return stats
...
def mismatch():
    ...
    for name,sc in scen.items():
        mpc=K.KoopMPC(km, w_N=K.W_N_OPERATING)                             # nominal weights, nominal model
    ...
    json.dump(out, open('results_cascade_mismatch.json','w'), indent=2)
    return out
...
def frontier():
    ...
    for bo in [0.0,0.7,1.5,2.2]:
        mpc=K.KoopMPC(km,w_N=K.W_N_OPERATING,backoff_nh=bo,dynamic_backoff=False)
    ...
    json.dump(dict(koopman=kf,cascade=cf),open('results_cascade_frontier.json','w'),indent=2)
    ...
    plt.tight_layout();plt.savefig('fig_cascade_pareto.png',dpi=140);plt.close(); print("fig_cascade_pareto.png saved")
