"""
koopman_mpc_bsm2.py  (v2)
MIMO Koopman-operator MPC for the ASM1-calibrated 5-tank BSM2-style cascade
(plant_model_bsm2.py). Preserves the convex-QP / EDMD-ridge / offset-free core.
...
class KoopMPC:
    def __init__(self, km, Np=12, w_air=1.0, w_pump=0.3, w_N=1.0, q_do=6.0, do_ref=DO_REF,
                 r_du=(3e-3,3e-3), rho=8e3, eps_s=1.0, backoff_nh=1.0, backoff_tn=1.0,
        ...
        self.r_du=np.array(r_du); self.rho=rho; self.eps_s=eps_s
        ...
        for _ in range(self.slp_iters):
            ...
            if self._prob is None:
                ...
                self._prob.setup(self.P,q,self.A_c,l,u,verbose=False,warm_start=True,
                                 eps_abs=1e-4,eps_rel=1e-4,max_iter=6000,polish=False)
            else:
...
# ------------------------- closed-loop runners -------------------------
def run_mpc(x0, mpc, days=8.0, delay_steps=1, forecast="noisy", fc_sigma=0.20,
    ...
    for k in range(n):
        ...
        x=PM.rk4_step(x,list(u),(S_S,NH_in,X_S,X_BH,Q),DT,nsub=4,scale=plant_scale)
        buf.append(x.copy()); buf.pop(0)
    return _finalize(log),x

def run_baseline(x0, ctrl, days=8.0, plant_scale=None):
    ...
    for k in range(n):
        ...
        x=PM.rk4_step(x,list(u),(S_S,NH_in,X_S,X_BH,Q),dt,nsub=2,scale=plant_scale)
    return _finalize(log),x
...
def metrics(log):
    ...
    # BSM-style Effluent Quality Index (nitrogen-dominant form) and Operational Cost
    # Index. EQI weights nitrogen loads (beta_TKN=beta_NO=20); OCI = aeration +
    # pumping + anoxic mixing (+ constant sludge term omitted as it is control-invariant here).
...
if __name__=="__main__":
    ...
    if stage in ("control","all"):
        x_ss=np.load('x_ss_bsm2.npy'); km=Koopman.load('koop_bsm2.npz')
        print("MIMO Koopman-MPC (emission-SLP) vs cascade-PI baseline...")
        mpc=KoopMPC(km,w_N=1.0)
        lk,_=run_mpc(x_ss,mpc,days=8.0,rng_=np.random.default_rng(3)); mk=metrics(lk)
        lb,_=run_baseline(x_ss,CascadePI(),days=8.0); mb=metrics(lb)
        ...
        json.dump(dict(Koopman=mk,CascadePI=mb),open('results_bsm2.json','w'),indent=2)
